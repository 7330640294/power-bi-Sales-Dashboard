...
#Top 3 High Ordered products
select productName,count(quantityOrdered) quantity from  products join  orderdetails using (productCode) 
group by  productName order by quantity desc limit 3;
#bottom 3 low ordered products 
select productName,count(quantityOrdered) quantity from  products join  orderdetails using (productCode) 
group by  productName order by quantity asc limit 3;
